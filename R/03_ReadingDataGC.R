...
p <- ggplot(DFg, aes(x=years1, y=eta1)) +
  ...
  facet_wrap(~ag.lab, 4, 5, scales="free_y")+
...
tim1 <- Sys.time()
OPT <- cleversearch(BICfun, lower=c(2, 4), upper=c(3, 5),
...
p <- ggplot(DF, aes(x=years1, y=eta1)) +
  ...
  geom_point(data=filter(DF, type=="Actual grouped"),
             aes(y=eta1, col=colo))+
  facet_wrap(~group, 4, 5, scales="free_y")+
  theme(legend.position = "none") +
...
p <- ggplot(DF, aes(x=years1, y=eta1))+
  ...
  geom_rect(data=DF[1:mg,], 
            aes(xmin = min(t2), xmax = max(t2), ymin = -Inf, ymax = Inf),
