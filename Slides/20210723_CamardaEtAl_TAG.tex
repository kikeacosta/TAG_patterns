...
\title[Modelling 2020 overall mortality by sex and age$\quad$]
{\LARGE{Modelling 2020 overall mortality \\by sex and age}} 
...
$\,$\\
\vspace{0.1cm}
\footnotesize{July 23th, 2021}} % (optional) 
...
\begin{itemize}
	...
	\begin{table}[]
		\begin{tabular}{l|c|c|c}
			...
			STMF                                          & 27                                  & 4.6                         & 18                         \\
			\rowcolor[HTML]{EFEFEF} 
			Statistical bureaus                           & \;\,\,\only<1>{7}\only<2->{\sout{7}} \only<2->{{\color{Red}6}} & 4.0 &78 \\
			\rowcolor[HTML]{FFFFFF} 
			Eurostat                                      & \;\,9 & 1.3                         & 19                         \\
			\rowcolor[HTML]{EFEFEF} 
			UN PD                        & 11                                  & 0.5 & 20 \\ \hline
			\rowcolor[HTML]{FFFFFF} 
			Totals                                        & \only<1>{67}\only<2->{\sout{67}} \only<2->{{\color{Red}66}}                                  & 6.6                         & 25                        
		\end{tabular}
	\end{table}
...
\item \only<1>{Sexes combined}\only<2->{\sout{Sexes combined}} \only<2->{{\color{Red}Including sex in the analysis}}
\end{itemize}
...
\begin{frame}[fragile]\frametitle{Model: general presentation}
	\vspace{-0.2cm}
	\begin{itemize}
		\item For each population over age $x$, log-mortality patterns:
		\begin{table}[]
			\begin{tabular}{l|c|c}
				& $<2020$                                        & $2020$                                         \\ \hline
				\rowcolor[HTML]{EFEFEF} 
				Females & {\color{Red}$\eta^{F1}(x)$} & {\color{Orange}$\eta^{F2}(x)$} \\
				Males & {\color{Green}$\eta^{M1}(x)$} & {\color{Cyan}$\eta^{M2}(x)$}
			\end{tabular}
		\end{table}
		\item In general log-mortality in 2020 is modelled as follows
		 $$
		 {\color{Red}\eta^{2}(x)} = {\color{Orange}\eta^{1}(x)} + {\color{violet}c} + {\color{OliveGreen}\delta(x)}
		 $$
		 \vspace{-0.5cm}
		 \begin{itemize}
		 	\item ${\color{violet}c}$ scaling factor
		 	\item ${\color{OliveGreen}\delta(x)}$ age-dependent adjustment component ($\sum {\color{OliveGreen}\delta(x)} = 0$)
		 \end{itemize}
	 \medskip
	 \item Both ${\color{Orange}\eta^{1}(x)}$ and ${\color{OliveGreen}\delta(x)}$ are assumed to be smooth
	 \medskip
	 \item We might assume males log-mortality as additive function of females log-mortality:
	 $$
	 {\color{Green}\eta^{M}(x)} = {\color{Red}\eta^{F}(x)} + {\color{Bittersweet}s(x)}
	 $$
	 \item Also ${\color{Bittersweet}s(x)}$ is assumed to be smooth
	 \end{itemize}	
\end{frame}
...
\begin{frame}[fragile]\frametitle{Model: including sex}
	\vspace{-0.2cm}
	\begin{itemize}
		\item We can include sex using different options:
		\begin{itemize}
			\item Sex-independent estimation [\emph{Stratified}]
			$$
			\left\{
			\begin{array}{lll}
			{\color{Orange}\eta^{F2}(x)} &=& {\color{Red}\eta^{F1}(x)} + {\color{Dandelion}c^{F}} + {\color{Dandelion}\delta^{F}(x)}\\
			{\color{Cyan}\eta^{M2}(x)} &=& {\color{LimeGreen}\eta^{M1}(x)} + {\color{SkyBlue}c^{M}} + {\color{SkyBlue}\delta^{M}(x)}
			\end{array}
			\right.
			$$
			\item Common 2020-factors + sex age-factor [\emph{Common}]
			$$
			\left\{
			\begin{array}{lll}
			{\color{Orange}\eta^{F2}(x)} &=& {\color{Red}\eta^{F1}(x)} + {\color{violet}c} + {\color{Orange}\delta(x)}\\
			{\color{Cyan}\eta^{M2}(x)} &=& {\color{LimeGreen}\eta^{M1}(x)} + {\color{violet}c} + {\color{Orange}\delta(x)}\\
			{\color{LimeGreen}\eta^{M1}(x)} &=& {\color{Red}\eta^{F1}(x)} + {\color{Bittersweet}s(x)}
			\end{array}
			\right.
			$$
			\item Sex-specific 2020-factors + sex age-factor [\emph{Saturated}]
			$$
			\left\{
			\begin{array}{lll}
			{\color{Orange}\eta^{F2}(x)} &=& {\color{Red}\eta^{F1}(x)} + {\color{Dandelion}c^{F}} + {\color{Dandelion}\delta^{F}(x)}\\
